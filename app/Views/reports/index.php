...
$tabs = [
    ...
    'proveedores' => ['Directorio', 'bi-building'],
...
<section class="reports-modern">
  <div class="module-hero reports-hero">
    ...
    <div class="module-actions">
      ...
      <a class="btn btn-outline-success" href="<?= $exportUrl('reports.providers.excel') ?>"><i class="bi bi-building"></i> Directorio</a>
  ...
  <form method="get" action="index.php" class="filter-panel reports-filter">
    ...
    <div class="reports-filter-grid">
      ...
      <label><span>Contraparte</span><select name="provider_id" class="form-select"><option value="">Todos</option><?php foreach (($providers ?? []) as $provider): ?><option value="<?= (int)$provider['id'] ?>" <?= (string)($filters['provider_id'] ?? '') === (string)$provider['id'] ? 'selected' : '' ?>><?= htmlspecialchars($provider['name'], ENT_QUOTES, 'UTF-8') ?></option><?php endforeach; ?></select></label>
      ...
      <label class="wide"><span>Buscar</span><input name="q" class="form-control" value="<?= htmlspecialchars($filters['q'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="Numero, contrato, contraparte, supervisor, factura u orden"></label>
  ...
  <?php if ($activeTab === 'analitica'): ?>
    <div class="reports-chart-grid">
      ...
      <section class="report-panel"><div class="panel-title-row"><h2>Contrapartes por clasificacion</h2></div><?php $barRows($analytics['proveedores_clasificacion'] ?? [], fn($r) => (float)($r['total'] ?? 0), fn($r) => htmlspecialchars($r['label'] ?? '', ENT_QUOTES, 'UTF-8'), fn($r) => (int)($r['total'] ?? 0), $maxTotal($analytics['proveedores_clasificacion'] ?? [])); ?></section>
      ...
      <section class="report-panel"><div class="panel-title-row"><h2>Top contrapartes por valor</h2></div><?php $barRows($analytics['financiero_proveedor'] ?? [], fn($r) => (float)($r['value'] ?? 0), fn($r) => htmlspecialchars($r['label'] ?? '', ENT_QUOTES, 'UTF-8'), fn($r) => $money($r['value'] ?? 0), $maxValue($analytics['financiero_proveedor'] ?? [])); ?></section>
  ...
  <?php elseif ($activeTab === 'inteligencia'): ?>
    ...
    <div class="reports-kpi-grid compact">
      ...
      <article><i class="bi bi-building"></i><span>Contrapartes activas</span><strong><?= (int)($summary['providers'] ?? 0) ?></strong></article>
    ...
    <div class="reports-chart-grid">
      ...
      <section class="report-panel"><div class="panel-title-row"><h2>Riesgo por contraparte</h2></div><?php $barRows($intelligence['provider_risk'] ?? [], fn($r) => (float)($r['risk_score'] ?? 0), fn($r) => htmlspecialchars($r['provider_name'] ?? '', ENT_QUOTES, 'UTF-8') . '<small>Vencidos ' . (int)($r['expired_total'] ?? 0) . ' - Rojo ' . (int)($r['red_total'] ?? 0) . '</small>', fn($r) => (int)($r['risk_score'] ?? 0), $maxScore($intelligence['provider_risk'] ?? [])); ?></section>
